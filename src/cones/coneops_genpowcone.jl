...
# Returns true if z is primal feasible
...
@inline function barrier_dual(
    ...
    res = mapreduce((i,j)->2*i*logsafe(j/i),+,α,(@view z[1:dim1(K)]))
    res = exp(res) - sumsq(@view z[dim1(K)+1:end])
    barrier = -logsafe(res) 
    @inbounds for i = 1:dim1(K)
        barrier -= (one(T)-α[i])*logsafe(z[i]/α[i])
    end
...
function update_dual_grad_H(
    ...
    phi = mapreduce((i,j)->2*i*logsafe(j/i),+,α,(@view z[1:dim1(K)]))
    phi = exp(phi)
...
#YC: need to check gradient_primal! and _newton_raphson_genpowcone
...
function gradient_primal!(
    ...
    if norm_r > eps(T)
        g1 = _newton_raphson_genpowcone(norm_r,p,phi,α,ψ)
        @. gr = g1*r/norm_r
